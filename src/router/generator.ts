...
export const generator = (routerMap, parent?): any[] => {
  return routerMap.map((item) => {
    const { title, icon, keepAlive, permissions, hidden } = item.meta || {}
    ...
    // 判断icon
    const componentToRender = Icons[icon] || icon
    currentRouter.meta.icon = componentToRender && renderIcon(componentToRender)
